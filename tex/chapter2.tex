...
下面我们来看具体的演化。
假设两个粒子间以矢量 $\boldsymbol{\xi} = \xi(\sin\theta \cos \phi \,\mathbf{e}_1+\sin\theta \sin \phi \, \mathbf{e}_2+\cos \theta  \,\mathbf{e}_3)$间隔，且以4-速度$u^\alpha$运动。
...
\begin{array}{r@{}l}
  \frac{{\rm d}^2 x^1}{{\rm d}t^2} &{}= \frac{1}{2}\frac{\partial^2 }{\partial t^2 }h_{+} \, \xi \sin \theta \cos \phi + \frac{1}{2}\frac{\partial^2 }{\partial t^2 } h_{\times} \,  \xi \sin \theta \sin \phi \\
  \frac{{\rm d}^2 x^2}{{\rm d}t^2} &{}= \frac{1}{2}\frac{\partial^2 }{\partial t^2 }h_{+} \, \xi \sin \theta \sin \phi + \frac{1}{2}\frac{\partial^2 }{\partial t^2 } h_{\times} \, \xi \sin \theta \cos \phi \\
   \frac{{\rm d}^3 x^2}{{\rm d}t^2} &{}=  0
\end{array}
\end{equation}
注意到，两个粒子的距离在$z$轴上的分量保持不变，这是因为引力波是横波，而我们选用的TT规范中，引力波的传播方向为$z$轴方向。
因此，在引力波传播方向上（$z$轴），引力波不会产生作用。
下面我们进一步简化，设两个粒子的连线与引力波波矢垂直，也就是说$\theta=0$，我们可以简化得到$\boldsymbol{\xi} = \xi( \cos \phi \,\mathbf{e}_1+\sin\phi \, \mathbf{e}_2+ 0\,\mathbf{e}_3)$
