...
\begin{myprop}{Plane Wave}{PlaneWave}
  % Schutz  p204
  在物理学领域，经常采用类似$A^{\alpha\beta} \exp({\rm i} k_\mu x^\mu)$形式的平面波解。
  当然，
\end{myprop}
...
我们可以选取具有如下形式的解
\begin{equation}\label{eq:xi_solution} 
  \xi_\alpha =  B_{\alpha} \exp({\rm i} k_\mu x^\mu)
\end{equation}
其中$B_{\alpha}$是常量，而$k^\mu$与引力波的平面波解中的
